...
function computePeriod() {
  ...
  if (corrLabelEl) corrLabelEl.textContent = "Mean pairwise correlation · " + riskSessions + " daily returns ending " + fmtDate(DATA.dates[rEnd]) + " · diagonal = within-sector";
}
...
function renderSectorReturnBars(container, rows) {
  container.innerHTML = "";
  const W = 560, rowH = 34, labelW = 170, padT = 8, padB = 24, padR = 64;
  ...
  rows.forEach((r, i) => {
    ...
    // up bars: label past the bar tip; down bars: label in the clear space right
    // of the zero baseline, so it can never collide with the sector names
    const val = el("text", { x: (isUp ? xEnd : zeroX) + 5, y: yTop + rowH/2 + 4, "text-anchor": "start",
...
/* ---------------- correlation matrix ---------------- */
const SECTOR_ABBR = {
  "Technology": "Tech", "Consumer Cyclical": "Cons Cyc", "Communication Services": "Comm Svc",
  "Healthcare": "Health", "Consumer Defensive": "Cons Def", "Industrials": "Indust",
...
function renderCorrelation(filtered) {
  ...
  const topEl = document.getElementById("corr-top"), lowEl = document.getElementById("corr-low");
  container.innerHTML = ""; topEl.innerHTML = ""; lowEl.innerHTML = "";
  if (filtered.length < 2) {
    container.innerHTML = '<p style="color:var(--muted);font-size:12.5px;">Select at least two companies to compare.</p>';
    return;
  }
  const rets = {}; filtered.forEach(t => rets[t] = PERIOD.metrics[t].retSlice);
  const secOf = (t) => DATA.stocks[t].sector;

  // all pairwise correlations once — feeds both the sector matrix and the pair lists
  const pairs = [];
  for (let i = 0; i < filtered.length; i++) for (let j = i+1; j < filtered.length; j++) {
    const a = filtered[i], b = filtered[j];
    const c = correlation(rets[a], rets[b]);
    if (isFinite(c)) pairs.push({ a, b, c });
  }

  /* --- sector × sector heatmap --- */
  const secs = [...new Set(filtered.map(secOf))].sort((x, y) => {
    const ix = TOP_SECTORS.indexOf(x), iy = TOP_SECTORS.indexOf(y);
    return (ix < 0 ? 99 : ix) - (iy < 0 ? 99 : iy) || x.localeCompare(y);
  });
  const agg = {};
  pairs.forEach(p => {
    const key = [secOf(p.a), secOf(p.b)].sort().join("|");
    (agg[key] = agg[key] || { sum: 0, n: 0 });
    agg[key].sum += p.c; agg[key].n++;
  });
  const avgCorr = (sa, sb) => {
    const e = agg[[sa, sb].sort().join("|")];
    return e ? { v: e.sum / e.n, n: e.n } : null;
  };

  const m = secs.length, labelSpace = 76, topSpace = 22;
  const cell = Math.min(52, Math.max(34, 380/m));
  const W = labelSpace + m*cell + 8, H = topSpace + m*cell + 6;
  const svg = el("svg", { viewBox: `0 0 ${W} ${H}` });
  secs.forEach((sec, i) => {
    const abbr = SECTOR_ABBR[sec] || sec.slice(0, 8);
    const colLbl = el("text", { class: "axis-label", x: labelSpace + i*cell + cell/2, y: topSpace - 8, "text-anchor": "middle", style: "font-size:9px;font-weight:600" });
    colLbl.textContent = abbr; svg.appendChild(colLbl);
    const rowLbl = el("text", { class: "axis-label", x: labelSpace - 6, y: topSpace + i*cell + cell/2 + 3, "text-anchor": "end", style: "font-size:9px;font-weight:600" });
    rowLbl.textContent = abbr; svg.appendChild(rowLbl);
  });
  secs.forEach((rowSec, ri) => {
    secs.forEach((colSec, ci) => {
      if (ci < ri) return;   // upper triangle only — the mirror half is pure repetition
      const e = avgCorr(rowSec, colSec);
      const x = labelSpace + ci*cell, y = topSpace + ri*cell;
      const fill = e ? divergingColor(e.v, 1) : cssVar("--surface-2");
      const rect = el("rect", { x, y, width: cell-2, height: cell-2, rx: 3, fill });
      svg.appendChild(rect);
      const txt = el("text", { x: x + (cell-2)/2, y: y + (cell-2)/2 + 3.5, "text-anchor": "middle",
        fill: e ? textColorFor(fill) : cssVar("--muted"), style: "font-size:10.5px;font-weight:600" });
      txt.textContent = e ? e.v.toFixed(2) : "—";
      svg.appendChild(txt);
      const hit = el("rect", { x, y, width: cell-2, height: cell-2, fill: "transparent" });
      hit.addEventListener("pointerenter", () => {
        const r = hit.getBoundingClientRect();
        const box = document.createElement("div");
        box.appendChild(ttHead(rowSec === colSec ? rowSec + " — within sector" : rowSec + " × " + colSec));
        if (e) {
          box.appendChild(ttRow(fill, "Avg correlation", e.v.toFixed(2)));
          box.appendChild(ttRow(cssVar("--muted"), "Stock pairs", e.n));
        } else {
          box.appendChild(ttRow(cssVar("--muted"), "Stock pairs", "not enough companies"));
        }
  ...
  const pairRow = (p) => {
    const row = document.createElement("div");
    row.style.cssText = "display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);font-size:12.5px;";
    row.title = DATA.stocks[p.a].name + " × " + DATA.stocks[p.b].name;
    const names = document.createElement("span");
    names.innerHTML = `<b>${p.a}</b> <span style="color:var(--muted)">×</span> <b>${p.b}</b>`;
    const val = document.createElement("span");
    val.style.cssText = "font-weight:700;font-variant-numeric:tabular-nums;display:inline-flex;align-items:center;gap:6px;";
    val.innerHTML = `<span style="width:9px;height:9px;border-radius:2px;background:${divergingColor(p.c,1)};display:inline-block"></span>${p.c.toFixed(2)}`;
    row.append(names, val);
    return row;
  };
  pairs.slice(0, 6).forEach(p => topEl.appendChild(pairRow(p)));
  [...pairs].reverse().slice(0, 6).forEach(p => lowEl.appendChild(pairRow(p)));
}
...
function buildPerfRows(filtered) {
  ...
  return filtered.map(t => {
    ...
    return {
      symbol: t, name: s.name, sector: s.sector, closes: s.close.slice(startIdx, endIdx+1),
...
function renderPerfTable(filtered) {
  ...
  document.getElementById("perf-sub").textContent =
    ...
    + " · click a sector to expand its companies";
  const rows = sortedPerfRows(filtered);
  const tbody = document.getElementById("perf-tbody");
  tbody.innerHTML = "";

  // group the sorted rows by sector (rows stay in sort order within their group)
  const groups = new Map();
  rows.forEach(r => { if (!groups.has(r.sector)) groups.set(r.sector, []); groups.get(r.sector).push(r); });
  const { startIdx, endIdx } = PERIOD;
  const glist = [...groups.entries()].map(([sector, list]) => {
    const avg = mean(list.map(r => r.ret));
    const finiteKey = list.map(r => r[perfSort.key]).filter(v => typeof v === "number" && isFinite(v));
    return {
      sector, list, avg,
      best: list.reduce((a,b) => a.ret > b.ret ? a : b),
      worst: list.reduce((a,b) => a.ret < b.ret ? a : b),
      hot: list.filter(r => r.rsi >= 70).length,
      cold: list.filter(r => r.rsi <= 30).length,
      sortKey: finiteKey.length ? mean(finiteKey) : null,
    };
  });
  // order sector bands by the active sort column's group average; fall back to avg return
  glist.sort((a,b) => {
    if (a.sortKey != null && b.sortKey != null)
      return perfSort.dir === "asc" ? a.sortKey - b.sortKey : b.sortKey - a.sortKey;
    return b.avg - a.avg;
  });
  ...
  const allExpanded = glist.every(g => singleGroup || perfExpanded.has(g.sector));
  document.getElementById("perf-expand").textContent = allExpanded ? "Collapse all" : "Expand all";
...
function exportPerfCSV() {
  ...
  const lines = [
    ["Ticker","Company","Sector",`Close ${startD}`,`Close ${endD}`,"Change %","Volatility % (ann.)","RSI 14","Vs 52W High %","Projected 1M trend %","Trend R2"].join(","),
    ...rows.map(r => [r.symbol, esc(r.name), esc(r.sector), r.startClose, r.endClose,
...
function wireEvents() {
  ...
  document.getElementById("perf-expand").addEventListener("click", () => {
    const secs = [...new Set(getFiltered().map(t => DATA.stocks[t].sector))];
    if (secs.every(s => perfExpanded.has(s))) perfExpanded.clear();
    else secs.forEach(s => perfExpanded.add(s));
    renderPerfTable(getFiltered());
  });
